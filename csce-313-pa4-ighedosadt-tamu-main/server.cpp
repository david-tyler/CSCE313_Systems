...
int main (int argc, char* argv[]) {
	...
	while (true) {
		...
		int new_socket = control_channel->accept_conn();
		if (new_socket == -1) {
            perror("accept");
            continue;
        }
		
		// Create a new channel with the accepted_socket (the socket is already established)	
		TCPRequestChannel* new_chan = new TCPRequestChannel(new_socket);
		// Dispatch a new thread to handle_process_loop
