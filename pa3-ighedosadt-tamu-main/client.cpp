...
void file_thread_function (/* add necessary arguments*/ BoundedBuffer* request_buffer, string filename, FIFORequestChannel* chan, int buffer_capacity) {
...
void worker_thread_function ( BoundedBuffer* request_buffer, BoundedBuffer* response_buffer,  FIFORequestChannel* chan, string filename, int buffer_capacity ) {
...
int main (int argc, char* argv[]) {
	...
	FIFORequestChannel* chan = new FIFORequestChannel("control", FIFORequestChannel::CLIENT_SIDE); //control channel
    ...
    if (f.empty()) {
        ...
        for (int i = 0 ; i < w ; i++) 
        {
            FIFOs.push_back(create_new_channel(chan));
        }

        // create `w` worker threads
        //worker_thread_function(&request_buffer, &response_buffer, FIFOs[0], f);
        for(int i = 0; i < w; i++) {
            worker_threads.push_back(move(thread(worker_thread_function, &request_buffer, &response_buffer, FIFOs[i], f, m)));
        }
